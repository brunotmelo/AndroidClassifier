package com.brunotmelo.acexample.util;
...
public class ExtAudioRecorder
{
    ...
    //samples added to return the output in doubles
    ...
    public double[] getSamples(){
        if (state == State.STOPPED){
            double[] dsamples = new double[samples.size()];
            for(int i = 0;i<samples.size();i++){
                dsamples[i]=samples.get(i);
            }
            return dsamples;
        }else{
            System.out.println("state not stopped");
            return null;
        }
